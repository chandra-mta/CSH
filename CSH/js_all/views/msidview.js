...
//------------------    
    ...
    render: function() {
        ...
        //--- list conditions
        limSet = this.model.get('sc')
        if(limSet){
            limSet = 'Choice: ' + limSet;
        }else{
            limSet = this.model.get('xlim');
            if(limSet !== undefined){
                limSet = 'Range: ' + limSet;
            }else{
                limSet = 'NA'
...
//        }
        ...
        var useStColor = true;
        var scheck     = this.model.get('scheck');
//
//--- for the case that there is not checking the limits or there is no limit 
//--- or possible list of selection available
//
        if (limSet === 'NA'){
        this.$el.css('background-color', computeColorDim("#AED6F1", this.model.get('limstep'))); 
        useStColor = false;

        }else if (scheck === 'WARNING'){
            //--- red warning
            this.$el.css('background-color', computeColorDim("#FF4500", this.model.get('limstep')));
            useStColor = false;

        } else if (scheck === 'CAUTION'){
            //--- yellow warning
    ...
    if (curVal === null){
        ...
        if (printfmt !== undefined) {
            ...
            if(! isNaN(Number(arg))){
                ...
                }else if(test >= 1.0 && test < 10){
                    printfmt = '%.3f'

                }else if(test >= 10.0 && test < 100){
                    printfmt = '%.2f'
