...
int main()
{
    ...
    while(1)
    {
        ...
        fp = popen("/data/mta/Script/SOH/soh_pcad_script.sh", "r"); 
        /* fp = popen("/data/mta/Script/SOH/copy_data_from_occ_part.py pcad", "r");  */

        /* check the script runs correctly */

        if (fp == NULL){
            fo  = fopen("/data/mta/Script/SOH/Logs/soh_pcad_log", "2");
        ...
        /* update the log file --- this is used to see whether the process is still running */
            ...
            fo  = fopen("/data/mta/Script/SOH/Logs/soh_pcad_log", "a");
    ...
    fprintf(fo, "\t\tnohup /data/mta/Script/SOH/soh_pcad_daemonize &\n\n");
